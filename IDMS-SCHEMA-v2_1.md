...
| `roughlog.js`       | Overview → Rough Log                                                      | Not Built   |
| `tasks.js`          | Operations → Tasks & Maintenance                                          | Not Built   |
...
| `vessel.js`         | Config → Vessel Setup (4 tabs incl. Stability Calculations)               | Partial     |
...
| `equipment.js`      | Config → Equipment Setup                                                  | Built       |
...
**Status:** Not built
**File:** `roughlog.js`
**Location:** Overview → Rough Log (also embedded as Tab 2 of Dashboard)

Single vessel-wide chronological log, department-tagged. Auto-populates from every significant system action; manual entries also permitted.

**Auto-populating event sources:**

- Fuel transfers and sounding updates
- Bunkering events
- Oily water separator runs
- Task completions (one-line summary from service report)
- Rounds completed / missed
- Trip events: open, close, fishery change, port of call
- Watch change (user login/logout as implicit handoff marker)
- Stability calculation saved
...
| `date`       | string | YYYY-MM-DD vessel local                                               |
| `department` | string | `engine` | `factory` | `deck` | `vessel`                              |
| `source`     | string | `system` | `manual` | `pwa`                                        |
| `user`       | string | Username or `system`                                                  |
| `category`   | string | Event type key (e.g. `fuel_transfer`, `task_closed`, `manual_entry`) |
| `body`       | string | The log line text                                                     |
| `ref_id`     | string | Optional reference to originating record                             |

**Storage:** OneDrive at `data/roughlog/roughlog-{YYYY-MM-DD}.json`, one file per day. Console ingests into SQLite `rough_log` table.

**Daily report slice:** entries where `date` matches report date, filtered to `engine` + `vessel` departments, ordered by `timestamp` ascending.
...
| `type`           | string   | `scheduled` | `unscheduled` | `deficiency`                                                    |
| `assets`         | string[] | Array of asset codes from `assets.csv` (batch jobs have multiple)                               |
| `skill_tags`     | string[] | Array of skill category keys; completing this task increments these on assignee's KSA profile   |
| `assigned_to`    | object   | `{ type: "role" | "direct", value: role_id | crew_id }`                                      |
| `recurrence`     | object   | `null` or recurrence rule (interval, unit)                                                       |
| `status`         | string   | `open` | `in_progress` | `closed`                                                             |
| `service_report` | object   | Populated on closure: work done, parts used, follow-up needed, requisition flag                  |
| `completed_by`   | string   | `crew_id`                                                                                        |
| `closed_at`      | string   | ISO 8601 UTC                                                                                     |

**On closure, a task:**

- Writes one-line entry to Rough Log per asset
- Updates asset maintenance history
- Increments `skill_tags` on assignee's KSA profile
- Creates a requisition reminder entry if `requisition_flag` is set

**Batch jobs:** single task, multiple assets. Service report duplicated to each asset's maintenance history independently.
...
- Distance to destination (nm) — great circle calculation
