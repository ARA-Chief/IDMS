...
**Version 2.48 — F/V Araho**

v2.48 *(2026-09-20)* — **§41.6d: an equipment note LEAVES the ordinary lists; §41.4f new: Recents, two mirrors of what was last edited.** The Equipment band has listed notes filed against a machine since v2.31.6, but it listed them *as well as* their department list — one note in two places, read twice and answered twice, which is the thing the Hub exists to stop. A note whose **type is Equipment and which names a machine** now leaves the lists it merely *fell into* — the department's own top level, General, and every personal list — for the pad. It **keeps the folder it was put in**: filing is an act, somebody chose `Rounds` or dragged a note onto `Overhauls`, and answering that by emptying the folder is the app overruling them. So such a note is in the pad, and in its folder if it has one, and nowhere else. It is the type and the code **together**, and neither half is negotiable. Keying off the code alone would empty `Alerts`, `To Order` and the `Offload List` — nearly every note in them names a machine, and they are lists people work off. Keying off the type alone would strand a note that has been set to Equipment but not yet given a machine: it would leave every list for a tree of codes with no code to hang under, and be reachable from nowhere. Such a note has simply **not moved**, and the picker under the radio is the one click that moves it. Both helpers live in the canonical reducer and are the single place either client asks; the reducer is unchanged otherwise and **no event, field or stored value changes** — this is entirely a question of which list draws which note. `notesReduce.hiddenByPad(note, folderPath)` is what a renderer asks about the list it is drawing; `isFiledAgainstEquipment(note)` is the narrower question of whether it moved at all, and what puts it in the pad. The reserved `Rounds` folder therefore keeps its rounds comments (§41.11) *and* they appear under their machines — two ways in to one note, which is not the thing "one home per note" was protecting against. That was two **records** of one job drifting apart; a note reachable from the round it was made on and from the pump it is about is still one note, with one completion. The pad itself is deliberately **wider** than the rule — it still lists every note carrying a code, including the assignable ones, so a machine's page is the whole of what has been said about it and nothing was taken from anywhere. **The Console gains the pad**, which it never had: the same tree, over `db.searchAssets` and a new `has_equipment` filter on `db:getNotes` rather than a replay in memory, so the two sidebars are finally the same sidebar. New **§41.4f Recents**: two rows, one vessel-wide closing the Global group and one per department below the reserved pads, each mirroring the last `RECENT_LIMIT` (10) notes edited, newest `updated` first. A mirror is **not a place** — nothing is filed there, no note is counted there, nothing can be dropped on it, the add bar is put away, and each row says which list it actually came out of. The §41.4c star order is deliberately **not** applied: the list answers "what has just been touched", and a star jumping the queue would stop it answering that. Tests: `tools/test/notes-views.test.js` (new — both rules over the page's own functions, plus a cross-repo check that neither client has grown a second opinion) and `IDMS-Console/tools/test-notes-views.js` (new — the same two rules over SQLite rows).

v2.47 *(2026-09-13)* — **§41.4: `note_starred` / `note_unstarred` may carry `reason`, and the Offload List keeps its order with stars.** The offload worklist is sections in an order that means something, and one note per row in a folder is a flat list, newest first. The Console now stars the open notes of the uppermost section with anything still open, and moves the stars to the next section when that one is done (IDMS-Console/docs/offload.md §9a); a `note_reordered` gives each its place in the section, so starred notes read in list order. A star written that way carries **`reason: "offload_order"`**, because stars are shared and a person may star an offload note for their own reason: the Console treats a star as its own only while the *last* star event on the note carries that reason, never removes anyone else's, and never re-stars a note whose last star event is somebody's unstar. `reason` is additive and optional — the reducer ignores it and neither client needs to write one — and clients star and unstar exactly as before. No reducer change.

v2.46 *(2026-09-12)* — **§41.4e: `Offload List`, the third reserved folder, and the second one that notifies by being landed in.** The Console's Operations → Offload screen used to raise every worklist row as a task at **Commence Offload**. It now pushes each open row into Engine Room → `Offload List` as its own **assignable** note — `origin: "offload"`, a note id derived from the trip and the row (`note-ofl-{trip}-{row}`) so two consoles pushing the same row land on one note — and keeps the row and the note in step both ways: ticking either completes the other, and concluding the offload archives what is still open with a comment saying why (IDMS-Console/docs/offload.md §9–10). A row that already pointed at a job in Due & Completed is still claimed as a task rather than copied into a note. `Offload List` is reserved on the same terms as `Shipyard` — **Engine Room only**, named by string from another screen, so neither client can add, rename, remove or drag it — and **a note in it alerts everyone in the engine department (§41.7)**, whether the Console pushed it, somebody filed it there, or it was dragged in; acknowledgement stays per user. **Notes there default to assignable**: a creation in the folder carries `assignment: "assignable"`, and a plain (unassigned) note re-filed into it is followed by `note_assignment_set {mode: "assignable"}` from the client that moved it. Equipment notes and notes with people on them keep what they are. No new event or field — `origin` is a free string and `assignment` on `note_created` was already there — so `notes-reduce.js` is unchanged. Tests: `tools/test/notes-tree.test.js` (the row, and the string agreeing across the page, the hub and both Console files) and `tools/test/notes-alerts.test.js` (new — who the hub tells, over the page's own `ntaComputeAlerts`).
...
v2.42 *(2026-09-09)* — **§41.6: promotion is handed over rather than described, and two things it was dropping now travel.** The notes page's **Promote to Task…** printed a sign — *open this note in the Console and use Promote to Task there* — which named a section the Console had since renamed, and which inside the **IDMS Notes window** was a dead end: that window is opened *by* the Console, so being told to go to the Console meant finding the same note again by hand in a list of hundreds. The button now **hands the note over**. IDMS Notes exposes one preload method (`window.idmsNotesHost.promote(note_id)`, origin-checked and id-shaped) which launches the Console binary with `--promote-note=<id>`; the Console takes a **single-instance lock** — it never had one, and a second Console means two writers on one SQLite file — so that either starts it or hands the argument to the one already running, which raises itself, opens the note in the list it actually lives in, and runs the same confirmation the Console's own button runs. **One promote flow, reached from either side**; the page still holds no copy of the task-creation form, which was the point of the original decision. In a browser or on a phone there is no host to hand to, `present` is absent, and the page says where the form is — naming the section it is under today. **What promotion was dropping:** the note's *assignees* and its *checklist*. Promote is offered on assignable notes only, so the holders are the one thing the form cannot infer from prose; they now arrive as the keys a task resolves — login where the person has one, display name where they do not, which is what `assigned_to` has always held. All of them ride in on `assignees` with the first in `assigned_to` (§41.6c), and a holder changed on the form discards the note's list as stale, by that section's own rule. A name written into a comment still wins, being the more recent statement — the same rule the equipment code already followed. The checklist has no field on a task, so it is written into the description **with its ticks intact**; writing it unticked would claim a job nobody had started. Since promotion tombstones the note, both were previously gone for good. Also: attachments are still copied under the new `task_id` *before* the form opens, deliberately, but a **cancelled** promote now deletes those copies rather than leaving them under a task that never existed, invisible to every screen and to the §41.14 sweep alike. `promoteHolders` / `promoteChecklist` are pure and live in the reducer beside the rest; Console-side harness `_notes-promote-harness.html`, tests `tools/test-notes-promote.js`.
...
| `gear_sizes` | object[] | no    | What the member wears: ordered rows `{ item, size, note }`, all free text (`item` required, capped 40 / 24 / 120 chars). Edited in the Console Crew List and the PWA's My Profile through the one mirrored module `utils/gear-sizes.js`. **Absent is not empty:** a record with no key is shown the default items (Gloves, Boots, Trousers, Jacket, Shirt, Sweatshirt, Coveralls, Rain Gear, Hat), filled from the 2021 engine room gear order for the eight people it names — that seed lives in the module, keyed by `crew_id`, not in this file, and becomes stored data on the member's first save. `[]` means every row was removed and is shown as such. |
...
|---|---|---|
| `note_created` | `{note_id, title, body?, scope, folder?, equipment_code?, assignment?, steps?, template_id?, origin, group_alert?, attachments?, alert?}` | `scope` = `{level: "general"\|"department"\|"crew", department?, owner_crew_id?}` — see §41.4a. `origin` = `manual` \| `template` \| `emergency_offline` \| `task_demote` (§41.6f) \| `alert` (§41.4e). `alert` is an additive block written only by the Console's alerts lane — `{rule_id, rule_name, rule_text, department, severity, audience, episode_key, fired_at, trace}` — carried whole through the reducer so the reason a rule fired travels with the note rather than being re-derived from a config that has since changed; every other note holds `null` and no reader has to know about it. `assignment` states the note type outright; without it a creation carrying an `equipment_code` is equipment-typed, which is what lets an importer say what it plainly meant in one event. `steps[]` = `[{step_id, text, equipment_code?}]`. `group_alert: true` settable only by the department head (§41.7). `attachments[]` = §41.4b. |
...
| `note_starred` / `note_unstarred` | `{note_id, reason?}` | Shared, not per-user: a star marks a note important for everyone, matching the hub's non-private premise. Starred notes sort above the rest in every view. `reason` is written only by a Console lane that stars on its own account — `"offload_order"` for the Offload List's order of operations (v2.47) — so that lane can tell its stars from a person's and never take off one it did not put on. Clients writing a star by hand omit it. |
...
### 41.4f Recents — a mirror, not a place

Two rows, on both clients, each showing the last **`notesReduce.RECENT_LIMIT`** (10) notes edited, newest first:

| Row | Where it sits | Reach |
|---|---|---|
| 🕓 **Recents — vessel-wide** | closing the **Global** group, under General notes | every note aboard, whoever wrote it and whichever department it belongs to |
| 🕓 **Recents** | under the reserved pads, above the folder band | this department's notes plus the General ones — the same reach the rest of the sidebar has |

**Recency is `updated`**, which every mutation in the reducer stamps, including the ones with nothing to show for themselves. A note that has never been edited falls back to `created`, so a stream written before `updated` existed still sorts somewhere sensible rather than at the bottom forever.

**The §41.4c order is not applied.** Everywhere else a star sorts above everything; here it must not, because the list answers *what has just been touched* and any other order stops it answering that. Pinning is likewise absent — there is no person whose list this is.

**It is a mirror, and the UI says so at every edge.** Nothing is filed here: the add bar is put away rather than disabled, because a note typed here would have to be filed somewhere and there is no honest answer to where. Nothing is *in* it either — the membership test returns false, which is what keeps a count off the row: ten rows of whatever was last touched is not a backlog to be worked down. Nothing can be dropped on it. Every note it shows still lives exactly where it lives, and each row carries a short phrase saying which list that is — `General`, `Engine Room · Alerts`, `Engine Room · William`, `Equipment` — because ten titles pooled from every list aboard with no context is a worse answer than no list at all.

**Completed and archived follow the screen's own rules.** A completed note is mirrored and renders under the collapsed *Completed* group, as in every other list; an archived one appears only with **Show archived** on. Equipment notes are mirrored too — the pad is where they *live*, and a flurry of rounds comments against a machine is exactly the thing worth seeing here.

### 41.4e Reserved folders — To Order, Offload List, Shipyard and Alerts

Four folder names are owned by something other than the person filing notes *(the Console also reserves `Chief Alerts`; the notes page does not draw that row yet)*. Each sits as its **own row directly under Department notes**, above the folder band, in the Console sidebar's order, and none can be added, renamed or removed by hand.
...
| `Offload List` | 🚢 Offload List | Engine Room only — the offload worklist is the engine room's | the Console's Operations → Offload screen, which pushes one assignable note per worklist row when an offload commences (`IDMS-Console/docs/offload.md` §9) |
...
**An alert is a note, and the pad is the notification.** A note landing in `Alerts` alerts **everyone in that department**, on the phone and in the Console — that is what the folder means, and it is the one folder where filing something *is* addressing it to people. There is no assignee, and no read gate: alerts carry an `audience` (`department` | `head`) which routes *attention* — which surfaces raise it — and never *visibility*. Anyone who opens the pad sees everything in it. Acknowledgement stays per user (`alert_acked`), so one hand dismissing an alert does not silence it for the watch coming on. Notes raised by hand in the pad behave identically and carry no `alert` block.

**The Offload List notifies the same way, and its notes default to assignable.** A note open in Engine Room → `Offload List` alerts **everyone in the engine department** (§41.7) — pushed by the Console at Commence, filed there by hand, or dragged in; the test is "open and in the folder", so a note moved in later is told about exactly as one pushed. It differs from `Alerts` in one respect: the list is work to be picked up, so its notes are **assignable** by default and appear in the Notes Tray. A creation in the folder carries `assignment: "assignable"`; a plain unassigned note re-filed into it is followed by `note_assignment_set {mode: "assignable"}` from the client that moved it. An equipment note keeps its type and a note with people on it stays theirs. The Console keeps each note and its worklist row in step (ticking either completes the other) and, when the offload concludes, archives what is still open with a comment saying why — archived, not deleted, so what the crew wrote on it survives.
...
| `assignees` | Holder — all of them, first in `assigned_to` (§41.6c). Keyed the way a task keys people: login where the person has one, display name where they do not. An `Assigned to:` line in a comment wins, being the more recent statement, and a holder changed on the form discards the note's list. Somebody the roster no longer carries falls back to the `assignee_username` the event carried, and is dropped if it carried none — a raw `crew_id` in `assigned_to` names nobody on any task screen. |
| `steps` | Description, as a `[x] / [ ]` block above the provenance. A task has no step list, so this is the only place the checklist survives; it keeps its ticks, because writing it unticked would claim a job nobody had started. |
...
**Attachments are copied, not referenced.** Every photo and document on the note and on its comments is fetched and re-uploaded under the task — images to `data/assets/pictures/{task_id}/` with their thumbnails, documents to `data/tasks/files/{task_id}/`. Referencing them would leave the task pointing at files on the *note's* retention clock (§41.14), which tombstoning has just started. The copy runs before the form opens, so a slow or failed copy is surfaced before anyone types rather than between Save and the task existing; the promoting client therefore fixes the `task_id` in advance and hands it to the form. **A promote that is then cancelled deletes those copies** (v2.42): nothing was created, so they sit under a `task_id` that never existed, where no screen can reach them and the §41.14 sweep — which clocks notes — never will either.

**Promotion is confirmed, and the confirmation says what stops existing** — that this note is deleted, how many attachments travel, who is carrying it, that the checklist is being flattened into the description, and which metadata was found in its comments. A note becoming a task and back again should be rare; neither direction is a gesture that should happen by accident (§41.6f).

**Availability.** Promote is offered on **assignable** notes only (§41.6a): a task is work somebody carries, and an unassigned note or one filed against a machine has nobody on it.

**Where the form is, and how the notes page reaches it** *(v2.42)*. The form lives in the Console, and the notes page still holds no copy of it — a third copy of task creation on that page is a separate decision, not a side effect of this one. What changed is what the page's button *does* about that. It used to print a sign saying to open the note in the Console and promote it there, which inside the **IDMS Notes window** is a dead end: that window is opened *by* the Console, so being sent to the Console means finding the same note again by hand.

So the page **hands the note over**:

| | |
|---|---|
| The page | Calls `window.idmsNotesHost.promote(note_id)` if it is there, and says where the form is if it is not. `present` is the feature test; in a browser or on a phone there is no host and the sentence is the fallback, not the behaviour. |
| IDMS Notes | Exposes that one method, and only that one, from a preload on the notes window — the window is otherwise deliberately without one (`notes-app.js`). Guarded twice: the caller's origin must be the page's own home origin, checked at the moment of the call because a frame can be navigated after load, and the id must be id-shaped. It launches the Console binary with `--promote-note=<id>`, detached. |
| The Console | Holds a **single-instance lock**. It never had one, and two Consoles means two writers on one SQLite file — so the launch either starts the Console or hands its argument to the one already running. Either way the Console raises itself, opens the note in the list it actually lives in, and runs the same confirmation its own button runs. A note that has not reached the Console's cache yet is synced for once before being given up on. |

**One promote flow, reached from either side.** The rule does not move because the request came from another window: a note that is not assignable is opened on its Note Type control and left there, which is the one click that makes it promotable.
...
| **Equipment** | an equipment search box | Filed against one component of the asset register by its `code` (§41.6d). | *Amended v2.48:* the **EQUIPMENT** pad of the Notes sidebar, plus the folder it was filed in if it has one — but **not** the department's top-level list, General, or anybody's personal list. Also the **Notes** section of that component in Console → Maintenance → Equipment. Named no machine yet, it has not moved at all. |
...
**The EQUIPMENT pad — where an equipment note LIVES.** *Amended v2.48, and on both clients now; the Console had no band at all before.* A note whose type is **Equipment** *and* which names a machine is in this pad, and out of the department's own top-level list, out of General and off every personal list. The point is that a machine's notes stop cluttering the list somebody reads for everything else, and that the same remark is not read twice and answered twice.

**A folder keeps what was filed into it.** The one list such a note does *not* leave is the folder it was put in. Filing is an act — somebody chose `Rounds`, or dragged the note onto `Overhauls` — and emptying that folder is the app overruling them. The top-level department list is the opposite: it is where a note sits when nobody has filed it anywhere, which is exactly the list an equipment note has no business in.

Both halves are required, and each guards against a different way of getting it wrong:

- **The type, not the code.** `equipment_code` answers *what a note is about* and says nothing about who it is for (§41.6a). An alert carries a code and is still the department's to read; an `Offload List` row carries one and is still work somebody picks up; an assignable note carries one so the service report it is promoted into arrives with the machine filled in. Moving every note that merely *names* a machine would empty the three pads people actually work off.
- **The code, not the type alone.** Choosing Equipment on the radios and walking away leaves a note typed for a machine that names no machine. It would have left every list for a tree of codes with no code to hang under — it would exist, and be reachable from nowhere. Until it names its machine it has **not moved**.

Two helpers in the canonical reducer, and they answer different questions — a renderer needs both:

| | Asks |
|---|---|
| `isFiledAgainstEquipment(note)` | Has this note moved at all? Membership of the pad. |
| `hiddenByPad(note, folderPath)` | Does the pad take it out of *the list I am drawing*? `folderPath` is the folder that list **is** — `null` for the department's top level, for General and for a person's list. |

Nothing about the event stream changes: this is purely which list draws which note, and a client that has not shipped it still reads every note correctly.

**The pad is wider than the rule.** It lists **every** note carrying a code, including the assignable ones that merely name the machine they are about — so a machine's page is the whole of what has been said about it. Only equipment-typed notes were *moved*; nothing else was taken from anywhere it was.

**`Rounds` is the case this was drawn around.** Every note the phone files there is a rounds comment against a machine, equipment-typed by the reducer on the spot (§41.11). They stay in the folder — reachable from the round they were made on — **and** appear under their machine in the pad and on that component's page in the Console. Two ways in to one note, which is not what "one home" was ever protecting against: that was two **records** of one job drifting apart, and this is one note with one completion, reached from either end.

**The tree.** The Notes sidebar carries the pad below Personnel, collapsed by default, built from the codes actually in use in that department (plus General-scope notes) *and their ancestors* — so `601.001.003` is reachable by walking `601` → `601.001` → `601.001.003` even when no note is filed at the two upper levels. Selecting a node lists notes at that code **and everything beneath it**, which is what a tree node means; the count on a node follows the same rule. Nodes are drop targets: dragging a note onto one files it against that code. Names come from the cached register; a code with no name still renders, because a missing lookup must not hide a note.
...
Alerts are **derived, never authored** — computed at read time from five sources: notes assigned to me and not completed; **anything open in my department's `Alerts` pad** (§41.4e — the folder is the address, so every member of the department is told); **anything open in the Engine Room's `Offload List`**, for members of the engine department (§41.4e); tasks newly assigned to me (vs. my last ack); department-level notes with `group_alert: true` (settable only by the department head). Dismissal appends `alert_acked` with the dismissed ids, so it syncs across the user's devices.

The pad is tested **before** `group_alert`, because an alarm-severity rule sets both and "group alert from {actor}" would name the account the Console wrote under rather than the rule that fired. `audience: "head"` is not yet honoured on the phone: a head-routed alert notifies the whole department there, which is annoying rather than wrong, while the Console's tile honours it.
...
**Moving stock from the sheet** (PWA, 2026-09-07). A count corrects the number on a shelf; it cannot correct *which* shelf. Both are found out the same way — standing in front of it — so the item pane under a sheet row carries **Move stock from ‹this space›**, opening the ordinary `transfer` (§42.7) with its origin already set to the space being walked. It is not a new event type and not a new arithmetic path: one movement carrying both halves, so it cannot half-fail. Before this, correcting a misplaced part meant leaving the screen, finding the item in Inventory and searching out a From location by hand — which is why stock stayed where the book wrongly said it was.

It is deliberately **not** tied to an open audit. `expected_items` is frozen when a session opens precisely so that stock arriving later cannot retrospectively turn a complete walk into a partial one, and a transfer is exactly such an arrival at the other end; moving something is not a claim to have verified it, so no `session_id` is attached. It also works from **Unlocalized Stock**, which is the point — moving an item out of it is how one of those 2,864 addressless items gets a home at all. On the phone the button sits under the space list rather than on the sheet row: the row is already a name, a figure, a box and a tick, and a fifth target on a 375px screen on a moving boat is a mis-tap waiting to happen. On the Console it sits in the detail pane's header beside **Open in Register**, where there is room for it.

**Both surfaces carry it**, and `prcMoveForm(kind, opts)` grew an optional `{item, from}` so Stock Location can drive the register's own form rather than a second copy of it. `tools/test/console-derive.test.js` now asserts that every action which *files an event* — count, tick, tick-rest, start, finish, abandon, move — exists on both surfaces, and names the two documented exceptions that may differ: the Console's tree pane, and Print.

**Printing a cabinet, or one shelf of it** (Console, 2026-09-07). The count sheet's scope is chosen in the print dialog, not inherited from whatever the screen is showing. A cabinet is walked shelf by shelf and signed for as a cabinet, and previously the only way to print the other one was to cancel, flip the screen's own scope toggle and start again — the dialog simply reported "including sublocations" and offered no say in it. The dialog now offers **just this space** or **this space and everything below it**, each with its own line and page count so a 2,864-row cabinet cannot be sent to a printer by accident, and the choice travels to the renderer in `opts.scope` rather than being read back off the screen: it decides the **Space** column, without which a count filed on a multi-bin sheet has nothing to land against, and the line the sheet is signed under. A leaf bin has nothing below it, so it is offered no choice.
...
**Department gate.** Procurement is offered to `permission_tier: "admin"`, to roles listed in `settings.approver_roles`, to any user carrying `"Procurement"` in `departments[]`, and to anyone in a department named by `settings.access_departments` — which ships as `["Engine Room", "Wheelhouse"]`. Same shape as `userIsPurser()`, generalised twice.
...
**The wheelhouse, and the purser decision inside it** (2026-09-07). `Wheelhouse` is a department on the crew records — Master, two Mates and two Pursers — though it appears in neither vessel's declared `departments[]` list. Admitting the department admits all five, and the pursers are the half that had to be decided rather than assumed: until this change `applyCrewNavVisibility` hid Procurement from the purser tier on the Console, with the note that opening it to them would be *its own decision*. That decision was taken, and the note is why it was taken deliberately rather than inherited from a list. A purser keeping the vessel's accounts has more call on what was bought than most of the people who could already see it — and reading the register commits nothing, which is the next paragraph.

**The Console followed.** `nav-procurement-group` is no longer hidden from the purser tier; Operations and Shipyard keep that rule, and only Procurement left it. The two surfaces must agree about who can see the stores register, so widening one without the other would have been the drift the mirrored reducer exists to prevent.
