...
**Version 1.10 — F/V Araho**

v1.10 *(2026-09-07)* — **Incinerator status: one replay, read completely, on every device — PWA 1.19.** The ER hub lamp had been lighting "running" for sessions the whole crew had stopped or deleted, on some phones for weeks; one phone this month showed a run of over 800 hours. That figure was never an hour-meter. It is time since the `start_time` of whichever session that device believed was still open — and the device believed it because it had replayed an **incomplete or unordered** copy of `data/incinerator/events/`. The forensics on the live log (196 files, May–Aug 2026) show every mechanism at work, and on both surfaces: the Console force-closed one session on four separate days, a day to six days *after* its deletion had been written (its reader replayed in download order until 2026-08-22 and dropped any end or deletion that arrived ahead of its start); a phone wrote two `session_end`s on 24 Aug against a session deleted on 5 Aug (it had a stale view — either a phantom from a silently-skipped file, or in-memory state from a fortnight earlier, since the phone never re-read the log unless someone navigated to the hub); one crew member wrote seventeen `session_end`s in ten seconds against a session already deleted (STOP was not disabled while its write was in flight, and OneDrive for Business does not list a just-written file immediately, so the screen kept saying ON); and several sessions carry `end(force), deletion, end(force), deletion` pairs seconds apart for the same read-your-writes reason. Both readers also skipped, silently, any event file whose fetch failed — 196 concurrent Graph GETs from a phone on a satellite link — and replayed the rest. One skipped deletion is one phantom "running" lamp. The phone's listing was additionally unpaginated (Graph pages `:/children` at 200 items; the folder holds 196).

**The replay is now one shared file: `utils/incinerator-reduce.js`, byte-identical with the Console's `src/renderer/js/incinerator-reduce.js`** (the `procurement-reduce.js` convention, enforced by `tools/test/incinerator.test.js`). It sorts by filename, collects every `session_start` first and applies everything else in log order, so an end or deletion can never be dropped for sorting ahead of its start — download order and clock skew between devices stop mattering. It reports `orphans` (events naming a session with no start) so a reader holding an incomplete log can tell. The aggregation of racing starts (earliest wins, quantities sum) and `session_correction` semantics are unchanged; a null `stop_time` in a patch still re-opens, and the *writer* now refuses to produce one.

**The reader's contract (both surfaces).** (1) The lamp, the timer bar and START/STOP are painted only from a **complete** replay. A refresh that cannot read every listed file fails as a whole and keeps the last complete state, saying so on screen (`⚠ Could not read the log — showing the state read at HH:MM`); until a complete replay has succeeded this page-load the lamp shows `?` and the button reads *STATUS UNKNOWN — TAP TO RETRY*. Never a partial log. (2) The folder listing follows `@odata.nextLink` with `$top=999`. (3) Event files are immutable, so once read they live in `localStorage` under `fw_incin_events` (§8) keyed by filename and are never fetched again: a refresh costs one listing plus the files this device has never seen, six at a time, with retry and backoff on network errors, 429 (honouring `Retry-After`) and 5xx. A file this device wrote is put in that cache at write time, so its own START / STOP / DELETE is on screen at once even while the listing is still catching up. Cached files the listing no longer has are forgotten after 24 h. (4) The log is re-read when the ER hub or the incinerator screen is shown, whenever the page becomes visible again, and every 60 s while either screen is on show or an open session is in memory. (5) **Nothing is written without a fresh complete read first, and that read must agree the write still makes sense**: START needs no open session; STOP and ADD need one; DELETE needs a live session; editing a stopped session must keep a stop time; a manual add with no stop while a session is open is refused; every confirm button is disabled while its write is in flight. When the read disagrees, the user is told *what another device did* and nothing is written. (6) A session open longer than 12 h is shown as `ON · STALE?` on the tile and `Incinerator running — stale?` on the bar (the Console's threshold); elapsed above a day reads `Nd HH:MM:SS`, so nobody again reads `828:14:03` as an hour-meter. The Console's `forceCloseSession` re-reads first and returns `{ ok:false, reason }` instead of writing an end against a session already stopped or deleted.

**No event writes were made to repair the log.** Replayed completely and in order it already holds no open session; every phone that reads it completely agrees. §8 gains the `fw_incin_events` row.
...
| `fw_incin_events` | `JSON { v: 1, files: { [filename]: event } }` | Global | Immutable incinerator event files already read from `data/incinerator/events/`, keyed by filename (v1.10). A refresh fetches only files not present here; files this device writes are added at write time. Entries the folder listing no longer has are dropped after 24 h. Never read without a fresh listing — it is a fetch cache, not a source of state. |
