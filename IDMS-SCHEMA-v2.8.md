...
v2.8 — ROS (Reliability Observation Study) module added (§39). Per-user factory log file gains `observations` array (schema_version 2). `capacity-{YYYY-MM-DD}.json` retired as a write target (legacy read retained). `capacity_observations` SQLite table gains `failure_mode_id`, `ros_session_id`, `source_user` columns. New IPC handlers: `db:ingestObservationsFromLog`, `db:saveObservationsToLog`, `db:getRosSessions`, `db:generateRosReportPdf`. Factory Production gains ROS tab (§39) and report generator. PWA gains observation push for factory users. Overview tab gains PWA observation overlay.
...
| `source` | string | `"pwa"` \| `"manual"` \| `"ros"` |
| `ros_session_id` | string \| null | UUID grouping all observations from one ROS submission. Null for non-ROS. |
...
The Factory Production module tracks daily midnight-MT production figures, capacity observations from factory equipment, and line section theoretical throughput. It is a 5-tab screen: **Overview**, **Today**, **Observations**, **ROS**, and **Setup**.
...
CREATE INDEX IF NOT EXISTS idx_cap_obs_ros     ON capacity_observations(ros_session_id);
```
...
| `db:getCapacityObservations` | `{ obs_date?, section_id? }` | `capacity_observations[]`. Accepts additional optional filters: `source_user`, `source`, `ros_session_id`, `from_timestamp`, `to_timestamp`. |
...
| `db:getRosSessions` | `{ trip_number? }` | `ros_session[]` ordered by `session_start DESC` |
...
getRosSessions:            (opts)    => ipcRenderer.invoke('db:getRosSessions', opts),
...
- Source badges: `[PWA]` (amber) for `source: "pwa"`, `[ROS]` (blue) for `source: "ros"`, no badge for `source: "manual"`.
- When source filter = ROS, rows are grouped by `ros_session_id` with a collapsible session header.
...
**FMEA tab** (Factory Production module: Overview · Today · Observations · ROS · **FMEA** · Setup)
...
## §39 — Reliability Observation Study (ROS)

**Status:** Built (v2.8)
**File:** `src/renderer/js/production.js` (ROS tab embedded in Factory Production)
**Screen key:** `factory-production` (ROS tab)
**Nav group:** Factory
...
A **Reliability Observation Study (ROS)** is a bounded time window of direct observation producing a reliability snapshot of the factory production line. Purpose: captures unreported failures and efficiency losses; compares structured observation against the continuous log baseline; enables time-bounded report generation. "Reliability Observation Study" (ROS) is the preferred term of art.
...
Key fields specific to ROS:
- `source: "ros"` — all rows in a ROS session carry this value.
- `ros_session_id` — UUID grouping all observations from one ROS submission. Null for non-ROS sources.
...
A ROS session is a batch of observations sharing a `ros_session_id` UUID. The UUID is generated at submit time and is immutable after submission. All rows in the session carry `source: "ros"`. Session metadata (start/end timestamps, section count, observation count) is derived from the observations themselves — there is no separate session header record.

Sessions returned by `db:getRosSessions` include:
- `ros_session_id` — UUID
...
- **Submit:** builds observation object with `source: "pwa"`, `ros_session_id: null`, UUID generated locally.
...
| `ros_session_id` | TEXT | UUID grouping a ROS session. Null for non-ROS. |
| `source_user` | TEXT | Username of the submitting user. |

Index added: `CREATE INDEX IF NOT EXISTS idx_cap_obs_ros ON capacity_observations(ros_session_id);`
...
| `db:getCapacityObservations` | `{ obs_date?, section_id?, source_user?, source?, ros_session_id?, from_timestamp?, to_timestamp? }` | `capacity_observations[]` — dynamic WHERE clause |
| `db:getRosSessions` | `{ trip_number? }` | `ros_session[]` ordered by `session_start DESC` |
...
**Modal** accessible from the ROS tab and via "Generate Report for this Session" on a read-only session view.

**Modal fields:** Time window (from/to datetime), source filter (`pwa` | `ros` | `manual` | all), section filter, incident inclusion toggle.
...
- Summary stat strip: total observations, ROS sessions, incidents included, avg observed rate.
- Timeline: sorted by timestamp; coloured markers by source (orange=PWA, blue=ROS, grey=manual); incident span bars.
...
- **Submit:** validates at least one row; generates `ros_session_id` UUID; sets `source: "ros"` on all rows; calls `appendObservationsToLogFile` for the current user and date; ingests to SQLite via `db:ingestObservationsFromLog`; reloads session list; shows newly submitted session.

**Read-only session view:**
- Non-editable table of all observations in the session.
- "Generate Report for this Session" pre-populates the report modal with `from = session_start`, `to = session_end`, `ros_session_id` filter.
